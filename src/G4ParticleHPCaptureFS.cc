...
// dice the photons
    ...
    if(getenv("SIMPLE_NEUTRON_CAPTURE_MODEL") &&
        ...
        for (i = 0; i < gammaEnergies.size(); i++) {
    ...
    if (HasFSData() && !getenv("G4NEUTRONHP_USE_ONLY_PHOTONEVAPORATION")) {
        ...
        for (it = products->begin(); it != products->end(); it++) {
            ...
            theOne->SetMomentum(
                    (*it)->GetMomentum().vect() * ((*it)->GetMomentum().t() - (*it)->GetExcitationEnergy()) /
                    (*it)->GetMomentum().t());
...
//101203 TK
        ...
        if (nPhotons == 1 && thePhotons->operator[](0)->GetDefinition()->GetBaryonNumber() == 0) {
            ...
            G4double Q =
                    G4IonTable::GetIonTable()->GetIonMass(static_cast<G4int>(theBaseZ), static_cast<G4int>(theBaseA),
                                                          0) + G4Neutron::Neutron()->GetPDGMass()
...
void
G4ParticleHPCaptureFS::Init(G4double A, G4double Z, G4int M, G4String &dirName, G4String &, G4ParticleDefinition *) {
    ...
    G4String tString = "/FS";
    G4bool dbool;
    G4ParticleHPDataUsed aFile = theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M, dirName, tString,
                                                  dbool);
